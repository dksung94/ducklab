...
DEFAULT_PROMPT = (
    ...
    "The response has a per-cell 'status' (ok/error) and 'outputs'. Text comes "
    "back inline; plots do NOT — they render in the human's browser and are "
    "elided from the API by default, which is what you want most of the time. "
    "Only when you actually need to SEE a figure yourself (e.g. to judge a plot "
    "you just made), add &images=files to the run/outputs call and Read the PNG "
    "path it returns — don't fetch images routinely. So the loop is: edit the "
    "file, run the cell(s), check status+text outputs, iterate; pull an image "
    "only when the decision depends on looking at it. "
